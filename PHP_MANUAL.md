...
3.  **Запустите сервер**:
    ...
    ./dropdpi-server
    ```
    _Порт `8443` должен быть открыт._
...
2.  **Настройте IP**:
    Откройте `cmd/client/main.go` и впишите IP вашего **Европейского сервера**:
    ```go
    var (
        serverAddr = "IP.АДРЕС.EU.СЕРВЕРА:8443" // <-- ВАЖНО
        serverKey  = []byte("...") // Ключ такой же, как на EU сервере
    )
    ```
3.  **Соберите клиент**:
    ```bash
