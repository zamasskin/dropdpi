...
- `enable_tls`: `true` для шифрования (WSS). По умолчанию `false` (без шифрования, обычный WS).
...
- `server_address`: Укажите реальный IP вашего VPS.
  - Пример для WSS (с шифрованием): `wss://1.2.3.4:443` или просто `1.2.3.4:443`
  - Пример для WS (без шифрования): `ws://1.2.3.4:80` (если на сервере отключен TLS)
